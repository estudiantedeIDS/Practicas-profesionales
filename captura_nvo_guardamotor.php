<?php   // Captura nuevo Guardamotor, almacenando datos en la base de Datos.
...
if(mysqli_num_rows($verificar_dispositivo) >0){
    echo'
      <script>
          alert ("Este Guardamotor ya esta capturado, verifique los datos por favor...");
          window.location = "../ingreso_nvo_guardamotor.php";
      </script>
...
if($ejecutar){ /*window.location(funcion de javascript) nos lleva a index.php despues de mostrar la alerta */
    echo'
         <script>
         alert("Guardamotor almacenado exitosamente");
    ...
    echo'
    <script>
    alert("Error: inténtelo de nuevo, Guardamotor no almacenado.");  
    window.location = "../menu_op_guardamotor";
    </script>
    ';
}
}else{
