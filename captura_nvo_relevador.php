<?php   // Captura nuevo Relevador, almacenando datos en la base de Datos.
...
if(mysqli_num_rows($verificar_dispositivo) >0){
    echo'
      <script>
          alert ("Este Relevador ya esta capturado, verifique los datos por favor...");
          window.location = "../ingreso_nvo_relevador.php";
      </script>
...
if($ejecutar){ /*window.location(funcion de javascript) nos lleva a index.php despues de mostrar la alerta */
    echo'
         <script>
         alert("Relevador almacenado exitosamente");
    ...
    echo'
    <script>
    alert("Error: inténtelo de nuevo, Relevador no almacenado.");  
    window.location = "../menu_op_relevador";
    </script>
    ';
}
}else{
