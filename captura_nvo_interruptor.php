<?php   // Captura nuevo Interruptor, almacenando datos en la base de Datos.
...
if(mysqli_num_rows($verificar_dispositivo) >0){
    echo'
      <script>
          alert ("Este Interruptor ya esta capturado, verifique los datos por favor...");
          window.location = "../ingreso_nvo_interruptor.php";
      </script>
...
if($ejecutar){ /*window.location(funcion de javascript) nos lleva a index.php despues de mostrar la alerta */
    echo'
         <script>
         alert("Interruptor almacenado exitosamente");
    ...
    echo'
    <script>
    alert("Error: inténtelo de nuevo,Interruptor no almacenado.");  
    window.location = "../menu_op_interruptor";
    </script>
    ';
}
}else{
    ...
    window.location = "../menu_op_interruptorr.php";
    </script>
    ';
}
